...
void CheckBASS()
{
	if (HelperFunctionsGlobal.LoaderSettings->EnableBassMusic == false)
	{
		MessageBox(WindowHandle,
			...
			L"Gnorc Cove: BASS Error", MB_OK | MB_ICONERROR);
	}
}
...
{
    PrintDebug("[GnorcCove] Loading basic model: %s... ", name);
...
{
	PrintDebug("[GnorcCove] Loading chunk model: %s... ", name);
...
{
	PrintDebug("[GnorcCove] Loading object animation: %s... ", name);
